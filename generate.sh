...
ogen --package api --target api --clean openapi/openapi.yaml

# Post-process: Fix ogen null handling bug (https://github.com/ogen-go/ogen/issues/1358)
if [ -f "api/oas_json_gen.go" ]; then
    echo ""
    echo "Post-processing: Fixing Opt* null handling..."
    go run github.com/plexusone/ogen-tools/cmd/ogen-fixnull@latest api/oas_json_gen.go 2>/dev/null || true
fi

# Post-process: Fix error body preservation
if [ -f "api/oas_response_decoders_gen.go" ]; then
    echo ""
    echo "Post-processing: Fixing error body preservation..."
    go run github.com/plexusone/ogen-tools/cmd/ogen-fixerror@latest api/oas_response_decoders_gen.go 2>/dev/null || true
fi
...
echo ""
echo "Generated files in api/:"
ls -la api/*.go 2>/dev/null | wc -l | xargs echo "  Total Go files:"
echo ""
echo "Next steps:"
echo "  1. Review changes in api/"
