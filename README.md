...
(`render.Provider`, `render.AudioUploader`) on top of this SDK, so bitHuman
video generation can be used behind the OmniAvatar abstraction. It depends
only on [`omniavatar-core`](https://github.com/plexusone/omniavatar-core).
...
Provider adapters live in the provider SDK repos so all bitHuman-specific
knowledge stays here; the real-time **live** adapter, which requires LiveKit,
lives in the batteries-included
